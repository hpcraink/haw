...
--
-- ATTENTATION:
-- To Prepare user, as MySQL-Root user create the haw user:
--     mysql -h localhost -u root -p
-- Then issue the SQL commands:
--     CREATE USER 'haw'@'localhost' IDENTIFIED BY 'paSSword';
-- This user must be granted access to the haw schema using (to run this script):
--     GRANT SELECT,INSERT,UPDATE,DELETE,CREATE,DROP,ALTER,REFERENCES ON haw.* TO 'haw'@'localhost';
-- For added security (without having to create/drop the following tables):
--     GRANT SELECT,INSERT,UPDATE,DELETE ON haw.* TO 'haw'@'localhost';
-- Please note: Checking JPA consistency upon deployment is
...
CREATE TABLE center (
    ...
    name VARCHAR(64) NOT NULL COMMENT 'The name of the compute center or an Owner',
    abbreviation VARCHAR(8) NOT NULL COMMENT 'The abbreviation of the compute center or the owner',
    version INT NOT NULL DEFAULT 1 COMMENT 'The version of this row, necessary for optimistic logging',
    PRIMARY KEY (id)
...
CREATE TABLE currency (
    ...
    namede VARCHAR(64) DEFAULT NULL COMMENT 'The official name in German',
    nameen VARCHAR(64) DEFAULT NULL COMMENT 'The official name in English',
    abbreviation VARCHAR(8) DEFAULT NULL COMMENT 'The international abbreviation',
    symbol CHAR DEFAULT '' COMMENT 'The symbol used for this currency',
    version INT NOT NULL DEFAULT 1 COMMENT 'The version of this row, necessary for optimistic logging',
    PRIMARY KEY (id)
...
CREATE TABLE person (
    id INT NOT NULL AUTO_INCREMENT COMMENT 'The unique ID of the person',
    firstName VARCHAR(60) NOT NULL COMMENT 'First name of this Person',
    lastName VARCHAR(60) NOT NULL COMMENT 'Last name of this Person',
    eMail VARCHAR(120) COMMENT 'E-Mail Address',
...
CREATE TABLE project (
    ...
    name VARCHAR(60) NOT NULL COMMENT 'Name of the project',
...
CREATE TABLE resourceuniversityshare (
    ...
    -- PRIMARY KEY (resourcetimespanID, universityID) -- this cannot be easily mapped in JPA-2.1
    UNIQUE (resourcetimespanID, universityID)
...
CREATE TABLE university (
    ...
    name VARCHAR(60) NOT NULL COMMENT 'The name of this University',
    abbreviation VARCHAR(8) NOT NULL COMMENT 'The abbreviation for the name of this University (like ST=Stuttgart, UL=Ulm, etc.)',
...
CREATE TABLE universitymembers (
    universityID INT NOT NULL COMMENT 'The unique ID of the university',
    personID INT NOT NULL COMMENT 'The unique ID of the person (which is a member of the university)',
    PRIMARY KEY (universityID, personID)
) CHARACTER SET utf8 COLLATE utf8_bin COMMENT='collection of members per university';
-- ----------------------------------------------------------------
